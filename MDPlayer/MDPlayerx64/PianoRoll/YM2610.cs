...
{
    public class YM2610(List<PrNote> lstPrNote, int MAXChip = 2) : BaseChip(lstPrNote, MAXChip)
    {
        ...
        public override void Analyze(int chipID, int dAdr, int dData, long vgmFrameCounter)
        {
            ...
            {
                ssgMul = Audio.ClockYM2610 / (float)masterClock;
            ...
            for (int i = 0; i < 6; i++)
            {
                if (i != 2 || (reg[chipID][0x000 * 0 + 0x27] & 0xc0) != 0x40)
                {
                    ...
                    Note[chipID][i] = YM2608MakeNote(i, vgmFrameCounter, ff, freq);
                ...
                for (int j = 0; j < 4; j++)
                {
                    ...
                    NoteCh3[chipID][j] = YM2608MakeNote(j, vgmFrameCounter, ff, freq);
        ...
        private static PrNote YM2608MakeNote(int ch, long startTick, float ff, int freq)
        {
