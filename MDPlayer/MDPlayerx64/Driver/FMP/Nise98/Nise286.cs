...
namespace MDPlayer.Driver.FMP.Nise98
{
    public class Nise286
    {
        ...
        {
            Log.WriteLine(musicDriverInterface.LogLevel.TRACE, "XCHG_SI_AX");
        ...
        private void SCASW()
        {
            if (!repSW || (repSW && regs.CX != 0))
            {
                ...
                Log.WriteLine(musicDriverInterface.LogLevel.TRACE, "SCASW AX:{0:X04} [ES:DI] val:{1:X04}", dsv, edv);
        ...
        {
            byte modrw = Fetch();
            Log.WriteLine(musicDriverInterface.LogLevel.TRACE, "LDS GW,EP modrw:${0:X02}", modrw);
        ...
        private void GRP2_EB_1()
        {
            ...
            switch (reg)
            {
                ...
                case 2://RCL
                    throw new NotImplementedException();
                case 3://RCR
                    Log.WriteLine(musicDriverInterface.LogLevel.TRACE, "RCR EB,1 modrw:${0:X02}", modrw);
                    uans = unchecked((byte)EB);
                    bool newCF = (uans & 1) != 0;
                    uans = (byte)((uans >> 1) | (regs.CF ? 0x80 : 0x00));
                    ans = (sbyte)uans;
                    regs.CF = newCF;
                    break;
                case 4://SHL
        ...
        private void GRP2_EW_CL(byte op)
        {
            ...
            if (count == 1)
            {
                switch (reg)
                {
                    ...
                    case 1://ROR
                        uans = unchecked(EW);
                        uans = (ushort)((uans >> 1) | ((uans & 0x01) == 0 ? 0 : 0x8000));
                        ans = (short)uans;
                        regs.CF = (uans & 0x8000) != 0;
                        regs.OF = (uans & 0x8000) != 0;
                        break;
                    case 2://RCL
                        throw new NotImplementedException();
                    case 3://RCR
            ...
            else
            {
                switch (reg)
                {
                    ...
                    case 1://ROR
                        uans = unchecked(EW);
                        uans = (ushort)((uans >> count) | (uans << (16 - count)));
                        ans = (short)uans;
                        regs.CF = (uans & 0x8000) != 0;
                        regs.OF = (uans & 0x8000) != 0;
                        break;
